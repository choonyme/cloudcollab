...
rfbSetProtocolVersion(rfbScreenInfoPtr rfbScreen, int major_, int minor_)
{
    ...
    if ((major_==3) && (minor_ > 2 && minor_ < 10))
    {
                     ...
                     rfbBool isUDP)
{
    ...
    /* VNCProxy - clients now have individual pointer events */
    cl->ptrAddEvent = NULL;
    cl->kbdAddEvent = NULL;
...
rfbProcessClientNormalMessage(rfbClientPtr cl)
{
    ...
    case rfbKeyEvent: /* edited by VNCProxy */
	if(cl->protocolMinorVersion < 9){
		n = rfbReadExact(cl, ((char *)&msg) + 1,
			   sz_rfbKeyEventMsg - 1);
		msg.ke.id = 0;
	}else{
		n = rfbReadExact(cl, ((char *)&msg) + 1,
				sz_rfbKeyEventMsgId - 1);
	}
	...
	if (cl->kbdAddEvent){
		cl->kbdAddEvent(msg.ke.down,                                			(rfbKeySym)Swap32IfLE(msg.ke.key), Swap16IfLE(msg.ke.id),		        cl);
	}
		
	rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbKeyEventMsg, sz_rfbKeyEventMsg);

//	if(!cl->viewOnly) {
//	    cl->screen->kbdAddEvent(msg.ke.down, (rfbKeySym)Swap32IfLE(msg.ke.key), Swap16IfLE(msg.ke.id), cl);
//	}

        return;


    case rfbPointerEvent: /* edited by VNCProxy */
	if(cl->protocolMinorVersion < 9){
		n = rfbReadExact(cl, ((char *)&msg) + 1,
				   sz_rfbPointerEventMsg - 1);
		msg.pe.id = 0;
	}else{
		n = rfbReadExact(cl, ((char *)&msg) + 1,			                                   sz_rfbPointerEventMsgId - 1);
	}
	...
	/* VNCProxy - call individual client pointer callbacks */
	if (cl->ptrAddEvent)
	{
		cl->ptrAddEvent(msg.pe.buttonMask,
				Swap16IfLE(msg.pe.x), Swap16IfLE(msg.pe.y),
				Swap16IfLE(msg.pe.id), cl);
	}
	
	rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbPointerEventMsg, sz_rfbPointerEventMsg); //New code?
	
	if (cl->screen->pointerClient && cl->screen->pointerClient != cl)
	    return;

	/* VNCProxy - remove logic for pointer "floor control" */
	/*
...
rfbProcessUDPInput(rfbScreenInfoPtr rfbScreen)
{
    ...
    case rfbKeyEvent: /* edited by VNCProxy */
	if ( (n != sz_rfbKeyEventMsg && cl->protocolMinorVersion <= 8)
		|| (n != sz_rfbKeyEventMsgId && cl->protocolMinorVersion == 9) ) {
    ...
    case rfbPointerEvent: /* edited by VNCProxy */
	if ( (n != sz_rfbPointerEventMsg && cl->protocolMinorVersion <= 8)
		|| (n != sz_rfbPointerEventMsgId && cl->protocolMinorVersion == 		9) ) {
	    rfbErr("rfbProcessUDPInput: ptr event incorrect length\n");
	    rfbDisconnectUDPSock(rfbScreen);
	    return;
	}
	cl->screen->ptrAddEvent(msg.pe.buttonMask,
		    Swap16IfLE(msg.pe.x), Swap16IfLE(msg.pe.y), 0, cl);
	break;
